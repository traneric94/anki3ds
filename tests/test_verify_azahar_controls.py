...
profiles\\1\\circle_pad="down:code$016777237$1engine$0keyboard,engine:analog_from_button,left:code$016777234$1engine$0keyboard,right:code$016777236$1engine$0keyboard,up:code$016777235$1engine$0keyboard"
profiles\\size=1
"""
    ...
    def test_cli_reports_errors_without_traceback(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            path = self.write_config(
                ...
                VALID_CONTROLS.replace(
                    'profiles\\1\\button_start="code:77,engine:keyboard"',
                    'profiles\\1\\button_start="code:13,engine:keyboard"',
                ),
            )
            stderr = io.StringIO()
            ...
            self.assertIn("button_start should map to keyboard M", stderr.getvalue())
