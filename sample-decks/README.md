...
`cards.tsv`, and `settings.tsv`, clean stale non-progress entries from tracked
sample folders, and preserve `state.tsv`/`review-log.tsv` progress and recovery
artifacts unless a fresh target is used.
